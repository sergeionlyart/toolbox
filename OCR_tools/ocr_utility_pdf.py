...
# ]
"""
Использование: mistral_ocr.py [OPTIONS] FILE_PATH

  Обработка PDF-файла с помощью API Mistral OCR.
  ...
  Варианты сохранения результата:
    - стандартный вывод (stdout, по умолчанию)
    - в файл (-o/--output)
    - в папку (-d/--output-dir):
        * HTML: создаёт index.html
        * Markdown: создаёт README.md

  Работа с изображениями:
    - Без изображений (по умолчанию)
    - Встроенные изображения (-i/--inline-images): как base64 URI
    - Извлечение изображений (-e/--extract-images): отдельные файлы (требует --output-dir)

Опции:
  --api-key TEXT         API-ключ Mistral (или переменная среды MISTRAL_API_KEY)
  -o, --output PATH      Путь к выходному файлу
  -d, --output-dir PATH  Директория для сохранения
  --model TEXT           OCR-модель Mistral
  -j, --json             Вернуть JSON-ответ вместо Markdown
  -h, --html             Конвертировать Markdown в HTML
  -i, --inline-images    Встроить изображения (для HTML/JSON)
  -e, --extract-images   Извлечь изображения отдельно (требует --output-dir)
  -s, --silent           Отключить вывод промежуточной информации
